"""Input-contract preflight for SIDInspector diagnostics."""
...
from sidinspector.metrics import alignment, collision, deployment_cost, head_tail_capacity, validate_inputs


def read_table(path: Path) -> pd.DataFrame:
    """Read a local table format supported by the preflight command."""
...
def build_metric_smoke_summary(
    ...
    """Run bounded D1-D5 smoke metrics and return compact rows."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate sid_assignments/item_metadata/interactions before running "
            "SIDInspector metrics. Optionally emits a bounded D1-D5 JSON smoke summary."
        )
    )
