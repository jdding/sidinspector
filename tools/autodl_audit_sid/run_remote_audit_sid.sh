...
PYTHONPATH=src "$PYTHON_BIN" tools/autodl_audit_sid/summarize_gate0_runs.py \
  ...
  --output _gate0_artifacts/autodl_runs/gate0_summary.csv || true
echo "Queue exit code: $EXIT_CODE"
date

echo "=== ARCHIVE TO AUTODL-FS ==="
mkdir -p "$ARCHIVE_DIR"
cp -r "$WORKSPACE/_gate0_artifacts/autodl_runs" "$ARCHIVE_DIR/" 2>/dev/null || true
cp -r "$WORKSPACE/docs" "$ARCHIVE_DIR/docs_snapshot" 2>/dev/null || true
