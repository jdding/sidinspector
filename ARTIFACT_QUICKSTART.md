...
- Expected runtime: typically under two minutes after dependencies are
  installed. Optional tests may skip when ignored upstream clones or local
  experiment caches are absent.
...
Open the anonymous URL in a browser, use the page's Download/ZIP entry, unzip
it, and run the verifier from the extracted directory. This browser-first path is
intentional: `anonymous.4open.science` may present a Cloudflare challenge to
`curl`, `wget`, or other command-line clients.

```bash
cd sidinspector-9BB2
