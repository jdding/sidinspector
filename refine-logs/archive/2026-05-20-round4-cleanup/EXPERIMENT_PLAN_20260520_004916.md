...
是 controlled stressor/diagnostic/baseline/under-stress 表，其中 variable-depth
行使用 max-depth 12,010 vs active 7,914，旧 Table 4 已删除，clean-checkout
...
| 3 | `variable_depth_cost_probe` | variable/long SID interface cost | D4, D5a, D7-boundary | `docs/VARIABLE_DEPTH_COST_PROBE.md`, `paper_assets/tables/table10_variable_depth_cost_probe.csv` | used as compact D5a boundary evidence, not D7 coverage |

2026-05-19 19:34 CST writing update:

- All three controller results are now integrated into
  `paper/sections/4_demonstration.tex` as method-inspired stressor evidence.
- `docs/PAPER_CONTROLLER_INTEGRATION.md` records the claim boundary: controller
  rows support diagnostic interpretation but do not add named-method coverage.
- No additional local or GPU experiment is required from the controller suite
