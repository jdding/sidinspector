...
| Alternative recent method | CapsID / DIG / AdaSID / CARD / AsymRec if artifact export is feasible | avoid shallow RQ-VAE + ReSID-only comparison if ReSID is not representative |
| Optional extension | DACT / CQ-SID-inspired artifact if cheap | drift or retrieval-facing extension, not CIKM v0 go/no-go |
...
1. 查找 RQ-VAE/TIGER-style baseline、ReSID、CapsID、DIG、AdaSID/CARD 或兼容实现；DACT 只作为 optional extension。
2. 确认每个实现是否能导出：
   - item-to-SID mapping；
...
| Recent tokenizer/codebook innovation | yes | ReSID / CapsID / DIG / AdaSID / CARD / AsymRec / DRIL |
