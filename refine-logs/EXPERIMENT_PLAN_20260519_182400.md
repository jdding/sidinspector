...
## Controlled Stressor / Controller Policy
...
job is narrower: check whether AUDIT-SID diagnostics react to known artifact
failure modes under controlled inputs.

Current v0 controller set:

| Controller | Backing row | Diagnostics | Paper role |
|---|---|---|---|
| `collision_collapse` | `sanity_mod_collision_hash` | D1/D2/D5a | artifact-table calibration for collapse |
| `semantic_only_grouping` | `sanity_category_prefix` | D2/D3/D5a | shows taxonomy purity and learned tokenization are not equivalent |
| `popularity_capacity_skew` | `sanity_popularity_balanced` | D3/D4/D5a | calibrates popularity/capacity and prefix fan-out |

Optional local follow-up:

- `qualified_collision_probe`: a D2b/D3 diagnostic that compares collision
  item pairs with popularity-matched non-collision pairs using current Musical
  interactions. It is useful only if it can reuse existing artifacts and stay
  outside named-method evidence.

Cut for v0 unless a specific claim requires it:

- synthetic capacity-budget sweeps;
- variable-depth/EOS trie-cost stressors;
- any controller named after, or presented as an approximation of, an
  unreleased paper method.
