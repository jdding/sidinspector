...
| A131 | Controlled stressor/controller selection | DONE | `docs/CONTROLLED_STRESSOR_SELECTION.md` | Controllers are separated from named-method coverage. v0 uses existing collision/category/popularity stressors as artifact calibration rows; `qualified_collision_probe` is the only optional local addition if needed. |
...
Controller-selection update: `docs/CONTROLLED_STRESSOR_SELECTION.md` freezes
the current controlled-stressor policy. Existing collision, semantic/category,
and popularity controllers can double-check D1-D5a finding interpretation, but
they remain a separate calibration table and must not be used as named method
coverage. The only near-term local addition worth considering is
`qualified_collision_probe`, a D2b/D3 interaction-qualified collision check
over current Musical artifacts.
