...
| B. Recent tokenizer / codebook innovation | tokenizer 服务 collaborative / ranking / sequence predictability，或直接优化 collision、utilization、soft routing、variable length | ReSID / DIG / AsymRec / DRIL / CapsID / AdaSID / CARD / DIGER | 必须覆盖一个代表性强且 artifact 可导出的 recent innovation；在表中标注 B1/B2/B3 facet |
| C. Continual / drift-aware tokenizer | new item、catalog shift、tokenizer refresh stability | DACT / SID staleness variants | optional；不能绑架 CIKM v0 |
| D. Industrial retrieval/search-oriented SID | semantic cluster ID, ranking alignment, beam/cost | CQ-SID / DIG retrieval framing / GenRec industrial variants | literature-motivated target；不作为 CIKM must-run |

因此，CIKM v0 的真正 must-run 是：

> Cluster A + Cluster B + sanity lower bound.

`RQ-VAE + ReSID` 只是当前候选组合，不是固定结论。如果 ReSID 无法导出可解释 artifact，必须寻找 Cluster B 内其他 facet 的替代 recent tokenizer；如果找不到，应该停止 CIKM 2026 投稿。
