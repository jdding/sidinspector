...
recovery. Across seeds 42--44, the GRID duplicate-\sid rate remains
0.8327--0.8421 and full-collision rate remains 0.9751--0.9769, so the collision
pressure is not a single-seed artifact.

\begin{table}[t]
\caption{Controlled stressors isolate diagnostic mechanisms. These rows are
not named-tokenizer evidence; they are checks that the audit reacts to the
failure mode each diagnostic is meant to expose.}
\label{tab:controller-stressors}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.25\columnwidth}>{\raggedright\arraybackslash}p{0.24\columnwidth}>{\raggedright\arraybackslash}p{0.39\columnwidth}@{}}
\toprule
Stressor & Target & Observed signal \\
\midrule
Qualified collision & D2b risk & co-occurring collisions: 3.86x vs hash: 1.19x \\
Capacity budget & D1/D4 pressure & head unique: 1.000; tail unique: 0.028 \\
Variable depth & D5a boundary & active-prefix cost differs from max-depth view \\
\bottomrule
\end{tabular}
\end{table}
...
cost need not match a fixed maximum-depth view.
