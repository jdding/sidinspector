...
\begin{table}[H]
\caption{Controlled stressors used to interpret the diagnostics. These rows are
not named-tokenizer evidence; they test whether D2, D4, and D5a react to
...
\toprule
Stressor & Observed diagnostic signal \\
\midrule
Qualified collision & GRID collided pairs have 3.86x co-occurrence lift;
hash collisions have 1.19x, separating raw collisions from qualified risk. \\
Capacity budget & Width-24 head-reserved codes keep head uniqueness at 1.0
but leave tail uniqueness at 0.028. \\
Variable depth & Active prefixes can differ from a fixed maximum-depth schema,
so D5a must report realized structure. \\
\bottomrule
\end{tabularx}
\end{table}
...
overlap user co-occurrence are a different object from synthetic hash
collisions. Second, collision-free addresses and collaborative-prefix alignment
...
prefix structure: head items can receive enough capacity while tail items
remain compressed, and active prefix cost need not match a fixed maximum-depth
view.
