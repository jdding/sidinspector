...
Table~\ref{tab:musical-diagnostic} exposes a split between addressability and
behavioral prefix alignment. The
...
The capacity ablation gives an intervention-style check: a capacity change fixes
one failure mode without repairing another. Increasing the GRID feature-text budget to
...
The D3 pattern is not confined to the Musical table: on an All\_Beauty 20k vertical, a
...
For external grounding, we also report bounded ranking context: a 5,000-user
Musical check uses SID prefixes only as candidate sets
