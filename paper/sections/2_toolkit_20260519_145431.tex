...
collision profile rather than a causal harm estimate. Interaction-qualified
collision harm, as motivated by qualification-aware collision
work~\cite{hu2026quasid}, is reserved for the next artifact version. The
current table still reports collisions because full code aliasing is an
artifact-level fact: two items assigned the same complete address cannot be
distinguished by a generator that emits only that address.
