...
The evaluation asks whether one adapter contract can turn heterogeneous \sid
exports into comparable profiles. GRID supplies a released GRID/RQ-KMeans-style
module; ReSID supplies processed Musical artifacts and a bounded GAOQ export
path. Sanity rows and controlled mechanism probes calibrate which differences
are structural, behavioral, or capacity-driven.
...
Table~\ref{tab:musical-diagnostic} puts the RQ-style rows together so capacity
and aliasing changes can be read before the ReSID contrast and the sanity
controls. The GRID ft row is labeled as feature-text because it uses processed
feature text from the ReSID artifact path and the released residual k-means
module, not a faithful raw-text TIGER reproduction. In this controlled setup,
the row exposes high aliasing pressure and limited tail capacity. The bounded
ReSID/GAOQ row exports one full code per item, yielding zero full-code
collisions and higher D3-L1 collaborative prefix recovery. The daggered rows
mark a structural floor: collision-free full codes are not by themselves
evidence of better recommendation quality.
...
pressure. The ablation is not a full item-unique leaf match, and additional
capacity could reduce aliasing further; the contrast is therefore an artifact
profile rather than a method ranking. The RQ-min row serves a different role:
it is a minimal residual-quantization reference adapter that passes the same
validator on the full Musical item universe. It shows that \tool accepts an
independent SID-generation code path, but it is not third named-method coverage
and does not reproduce TIGER, GRID, or ReSID.

\paragraph{Diagnostic findings.}
Table~\ref{tab:musical-diagnostic} separates addressability from behavioral
prefix alignment. The ReSID row is structurally item-unique; the category-prefix
control is also alias-free because it uses metadata categories as early levels
and a within-prefix item index as the leaf. It is a taxonomy control rather
than a learned tokenizer. D3 exposes the surprising part: the non-learned
category-prefix row recovers more level-1 co-occurrence neighbors (0.447) than
ReSID (0.154) or either GRID row (0.055--0.080). The point is not that category
IDs are better tokenizers, but that learned or item-unique addresses do not
guarantee behaviorally aligned prefixes. Direct mapping inspection makes that
failure mode visible.

The capacity ablation gives an intervention-style check: one failure mode can
improve while another persists. Increasing the GRID feature-text budget to
32/1280/1280 raises unique full codes from the 3.7--4.0k range to 9,874 and
reduces D2 aliasing from 0.977 to 0.779. This weakens a simple ``too little
capacity'' objection, but it does not make the row item-unique.
...
reported separately. The controlled probes in Table~\ref{tab:mechanism-probes}
give the same separation under simpler inputs: qualified aliasing separates
...
The D3 pattern is not confined to Musical. On an All\_Beauty 20k vertical, a
...
For external grounding, a 5,000-user Musical check uses SID prefixes only as
candidate sets and applies the same train-only co-occurrence/popularity reranker
to all six rows; D3 correlates with candidate recall (Spearman 0.943) and
...
constructed split keep it as supplementary directional evidence. The GRID
export path also extends to Sports: a real 20k GRID export
