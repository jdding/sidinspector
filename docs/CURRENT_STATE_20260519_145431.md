...
10. No new experiment is immediately required for the figure/table integration;
   the next empirical extension would be D7 only if a tokenizer/generator
   artifact exposes `generator_outputs`.
11. External simulated review Round 1 found no new experiment blocker for 8/10.
    The remaining package gate is commit/tag/push plus clean-checkout verifier.
