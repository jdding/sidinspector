...
**2026-05-19 taxonomy refresh**：Use
`docs/SID_CLUSTER_DIAGNOSTIC_REFRESH.md` for the current paper-facing method
taxonomy and D1-D7 coverage plan. This file remains the Gate 0A evidence/scoring
record, but the newer refresh refines Cluster B into B1-B4 facets and treats D7
generator/retrieval behavior as an interface hook rather than current v0
evidence.
...
## Current Evidence Update on 2026-05-19 03:13 CST

The audit framework is now populated with real local/AutoDL evidence. The
project should use `docs/GATE0A_EVIDENCE_MATRIX.md` as the current decision
record.

| Method | Cluster | Current evidence | Score | v0 decision |
|---|---|---|---:|---|
| GRID official-module RQ-KMeans | A | All_Beauty 5k local plus AutoDL 20k seeds 42/43/44 and 50k seed42, all join-valid with D1-D5a | 10.0 | main Cluster A evidence |
| ReSID balanced GAOQ | B | Musical_Instruments 23,742-item real GAOQ mapping with D1-D5a and D3v2 | 10.0 | main Cluster B smaller-dataset evidence |
| ReSID Sports exact balanced GAOQ | B | FAMAE checkpoints exist, but exact GAOQ was stopped with no mapping due CPU-bound constrained k-means | 4.0 | failure/provenance note, not blocker |
| CARD compact feature proxy | B/control | Sports compact e5 metrics exist, but source repair uses nearest-neighbor VQ and is not faithful CARD Sinkhorn | 7.5 | controlled stressor only |
| DIGER | B/D | public reference repo exists but no sprint-ready full export path in current audit | 4.0 | literature/backup only |
| Sanity baselines | sanity | Musical_Instruments category/hash/popularity lower bounds with D1-D5a and D3v2 | 9.0 | required lower bound |

Set-level interpretation:

- `Cluster A + Cluster B + sanity` is now satisfied for a conservative
  resource-demo framing.
- The current evidence is not a same-dataset GRID-vs-ReSID benchmark.
- The paper must present ReSID as smaller-dataset Cluster B evidence and should
  use ReSID-vs-sanity on Musical_Instruments as the clean same-dataset
  diagnostic case study.
- CARD must not be described as faithful CARD unless an upstream-complete path
  replaces the current compact feature proxy.

## Current Status

This audit framework is populated enough to close the core Gate 0A resource-demo
question. Remaining work is paper-readiness tightening, not waiting for Sports
GAOQ:

- update the paper-facing method coverage table;
- freeze the case-study wording around cross-dataset method coverage;
- decide whether CARD appears only as controlled stressor/backlog or is removed
  from main tables;
- avoid any claim that Sports ReSID exact balanced GAOQ completed.
