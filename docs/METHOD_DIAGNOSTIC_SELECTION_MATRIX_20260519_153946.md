...
Timestamp: 2026-05-19 15:39:46 CST
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| GRID / RQ-KMeans | A canonical SID | real exports: All_Beauty 5k/20k/50k; Musical feature-text controlled row | R | R | R | R | R | - | G | capacity/collision pressure; seed stability; same-item contrast vs ReSID | MAIN | surface 20k seed range and keep Musical caveat explicit |
...
| CARD faithful path | B2 capacity/collision | source repaired, but current compact feature run is not faithful named evidence | S | S | S | S | S | - | G | non-uniform quantization could stress D1/D2/D4 if faithful export works | HIGH | screen whether repaired source can produce a faithful item-to-SID export without proxy wording |
| QuaSID | B2 collision qualification | literature/current local code status unverified | S | S | S | S | S | - | - | strongest fit for D2b interaction-qualified collision harm | HIGH | verify paper/repo/code release and whether full/prefix conflict labels can be exported |
| AdaSID | B2 adaptive capacity | literature/current local code status unverified | S | S | S | S | S | - | - | adaptive capacity/collision should produce clear D1/D2/D4 contrast | HIGH | verify public artifact/code; prefer if item-to-SID export is direct |
| CapsID | B2 + B4 variable/soft SID | no confirmed runnable evidence in current local audit | S | S | S | S | S | - | G | variable/soft routing could make D4/D5a visibly different | MED | screen only after QuaSID/AdaSID/CARD faithful path |
| DIGER | B3 ranking/differentiable | public reference appears incomplete in current audit | S | S | S | S | S | - | G | ranking-aligned SID is the best bridge to D3/D7 if outputs exist | MED | re-check release status; run only if item-to-SID export is available without full generator training |
...
|---|---|---|---|
| True B2 evidence | current B2 is mostly proxy/literature; D2/D4 findings need real collision-aware methods | collision/utilization/capacity | QuaSID, AdaSID, faithful CARD, CapsID |
...
1. **B2 first**: screen QuaSID, AdaSID, and faithful CARD because they should
   directly produce D1/D2/D4 findings and strengthen the current diagnostic
   story without needing generator outputs.
2. **B3 second**: screen DIGER and joint search-rec SID only if they expose
