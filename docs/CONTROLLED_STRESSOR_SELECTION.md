...
useful for double-checking whether D1-D5a respond to known method-family
...
## Planned Method-Inspired Controllers
...
These rows remain useful, but they are no longer the whole controller story.
They are generic controls, not method-inspired controller evidence.

| Generic row | Existing implementation | Diagnostic target | Current role |
|---|---|---|---|
| `collision_collapse` | `sanity_mod_collision_hash` | D1/D2/D5a | Lower-bound collision calibration; confirms D2/D5a catch full collapse |
| `semantic_only_grouping` | `sanity_category_prefix` | D2/D3/D5a | Category-derived upper-control; shows metadata purity is not the same as learned tokenization |
...
- if space allows, summarize method-inspired controllers as one compact
...
- prioritize `qualified_collision_probe` and `capacity_budget_sweep` in the
  narrative; use `variable_depth_cost_probe` only if it produces a clean D5a
  boundary result.

Artifact repository:

- keep a dedicated method-inspired controller table;
- include the method-name boundary in the table caption;
- link each controller to D1-D5a and the related finding it validates;
- keep the older generic sanity rows as calibration rows, not controller
  substitutes.
...
- "AUDIT-SID includes method-inspired controlled stressors for collision
  qualification, capacity pressure, and variable-depth cost."
- "These stressors test whether D1-D5a respond to known SID tokenizer failure
  modes."

Not allowed:

- "AUDIT-SID reproduces QuaSID/AdaSID/CARD/CapsID/ACERec."
- "Controller results are evidence for named method quality."
- "Synthetic stressors solve the method coverage breadth gap."
