...
**2026-05-19 update**：`docs/SID_CLUSTER_DIAGNOSTIC_REFRESH.md` supersedes
this early taxonomy where it conflicts. The current paper-facing taxonomy keeps
Cluster A canonical SID, splits Cluster B into B1-B4 facets, keeps Cluster C for
drift/staleness, keeps Cluster D for deployment/search surfaces, and adds an R0
reference-only layer for evaluation/identifier foundations.
...
- **B1 recommendation-native / predictability-aware**：ReSID、DIGER、AsymRec；
- **B2 collision / utilization / adaptive codebook**：CARD、CapsID、AdaSID、variable-length SID variants；
- **B3 ranking / retrieval-aligned tokenizer**：DIGER、CQ-SID-inspired public artifact if available。
...
**CIKM v0 状态**：must-have cluster。v0 只需要一个代表性强、artifact 可导出的 Cluster B 方法；但 Method Coverage Table 必须标注它覆盖 B1/B2/B3 哪些 facet。优先 ReSID，因为代码和 processed dataset 更现实；如果 ReSID artifact 不可解释，优先审计 CARD，而不是盲目扩展到一串未确认代码的 2026 preprint。DIGER 只作为低优先级 backup；CapsID / AdaSID / AsymRec 当前按 future support 处理，除非发现可运行代码。
...
| Recent innovation | Cluster B | ReSID | CARD first; DIGER backup; CapsID/AdaSID/AsymRec only if code appears | 找不到可解释 artifact 则停止 |
...
### 必评轻量版：D5 Deployment-Cost Proxy

适用 cluster：A, D。

纳入条件：

- D5a required-light：只需 item-to-SID mapping；
- D5b optional：能导出 candidate list、beam outputs 时再加。
