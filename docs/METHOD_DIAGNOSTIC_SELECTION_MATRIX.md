...
| CARD faithful path | B2 capacity/collision | original NU-RQ-VAE path is export-shaped but as-is import/package path is broken; current repaired runner is proxy only | P | P | P | P | P | - | G | non-uniform quantization could stress D1/D2/D4 if faithful export works | MED | repair original `nu-rq-vae` only if time remains; current proxy stays out of main evidence |
...
1. **B2 first**: the first screen found QuaSID/AdaSID/CapsID to be paper-only
   for the current sprint and CARD to be proxy-only unless the original
   `nu-rq-vae` path is repaired. Do not add them to main evidence yet.
