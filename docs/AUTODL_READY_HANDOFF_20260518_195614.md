...
Current recommended queue is **quick only** until CARD/Cluster-A source is repaired. Robust/sweep/quality now hard-stop by default when CARD source is incomplete, because ReSID-only robust cannot pass Gate 0.
...
Purpose: preferred first serious batch on RTX 5090 **only after CARD/Cluster-A source is repaired**. With the current incomplete CARD clone, this queue refuses to launch unless `ALLOW_RESID_ONLY=1` is explicitly set for ReSID-only debugging.
