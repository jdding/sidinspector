...
- Paper-writing stance: the latest draft now uses a five-section Resource Track
  structure, expands cited references to 27 entries, preserves the same
  artifact-only claim boundary, and uses three main tables rather than the
  older Table 4 artifact-checklist layout.
...
- `paper/main.pdf` compiles to 5 pages total with generated vector Fig. 1,
  redesigned Table 1, D5a-aware compact Table 2, and controlled-stressor
  Table 3.
...
- `paper/sections/4_demonstration.tex` contains the same-item Musical
  diagnostic table and the controlled-stressor table.
