...
| Controlled stressor selection | `docs/CONTROLLED_STRESSOR_SELECTION.md` | Controller/stressor policy; calibration rows only, not named-method coverage |
...
- `docs/CONTROLLED_STRESSOR_SELECTION.md` records the controller policy:
  collision/category/popularity stressors are artifact calibration evidence,
  not method coverage; `qualified_collision_probe` is the only near-term
  optional local addition.
...
| Controlled stressors/controllers | selected for calibration only; not method evidence | `docs/CONTROLLED_STRESSOR_SELECTION.md`, `paper_assets/tables/table3_sanity_controls.*` |
