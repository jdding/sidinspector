...
- The CARD preflight checks source/import/export contracts only. It does not
  prove faithful CARD evidence until a real CARD input/checkpoint/export joins
  to AUDIT-SID metadata and interactions.
...
- Synthetic export on CPU preserved `ItemID` and emitted a `(6, 2)` code
  matrix. This is still preflight evidence, not paper evidence.
