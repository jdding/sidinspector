...
| CARD faithful path | B2 capacity/collision | original NU-RQ-VAE source/import/export-contract preflight passes locally via path/runtime-only overlay; no real CARD item universe export yet | S | S | S | S | S | - | G | non-uniform quantization could stress D1/D2/D4 if faithful export works | MED | next gate is real CARD input/checkpoint/export plus AUDIT-SID join; current preflight stays out of main evidence |
...
1. **B2 first**: the first screen found QuaSID/AdaSID/CapsID to be paper-only
   for the current sprint. CARD original `nu-rq-vae` now passes a local
   source/import/export-contract preflight, but it still needs real CARD input,
   checkpoint, item-to-SID export, and AUDIT-SID joins before main evidence.
   Do not add it to main evidence yet.
